...
export default function InstructorAnnouncementsPage() {
  ...
  const { pinnedNotices, regularNotices } = useMemo(() => {
    // Filter for instructor-relevant announcements (all, instructor, or undefined)
    const instructorAnnouncements = mockAnnouncements.filter(
      (a) => a.audience === "instructor" || a.audience === "all" || !a.audience,
    );

    const pinned = instructorAnnouncements.filter((a) => a.isPinned);
    const regular = instructorAnnouncements.filter((a) => !a.isPinned);

    // Sort: urgent first, then by date (latest first)
    const sortByPriority = (
      a: (typeof mockAnnouncements)[0],
      b: (typeof mockAnnouncements)[0],
    ) => {
